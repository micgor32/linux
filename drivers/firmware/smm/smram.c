...
static int smram_driver_probe(struct coreboot_device *dev)
{
    ...
    smram = kmalloc(sizeof(struct smram_info) + sizeof(struct smram_descriptor) * (smram_cbtable_info->number_of_smm_regions - 1), GFP_KERNEL);
    ...
    for (int i = 0; i < smram->nr_of_smm_regions; i++) {
        smram->descriptor[i].physical_start = smram_cbtable_info->descriptor[i].physical_start;
        smram->descriptor[i].cpu_start = smram_cbtable_info->descriptor[i].physical_start;
        smram->descriptor[i].physical_size = smram_cbtable_info->descriptor[i].physical_size;
        smram->descriptor[i].region_state = smram_cbtable_info->descriptor[i].region_state;
    }
